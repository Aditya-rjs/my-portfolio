          ...
          <div className="space-y-6 sm:space-y-8">
            {experiences.map((exp, idx) => (
              <motion.div key={idx} variants={fadeInUp} className="relative sm:pl-20">
                {/* Timeline dot */}
                <div className="absolute left-4 sm:left-8 top-5 -translate-x-1/2 hidden sm:block">
                  <div className="w-3 h-3 rounded-full bg-blue-500 border-2 border-background shadow-sm" />
                </div>

                <div className="group p-4 sm:p-6 rounded-xl border border-border bg-card hover:border-blue-500/30 transition-all duration-300">
